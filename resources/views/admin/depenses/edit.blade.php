{{-- resources/views/admin/depenses/edit.blade.php --}}
@extends('layouts.admin')

@section('title', 'Modifier la dépense - La Réserve Naturelle')
            ...
            <div style="background: white; border-radius: 12px; box-shadow: 0 4px 15px rgba(0,0,0,0.08); border: 1px solid #e8e0d5; padding: 35px;">
                <form action="{{ route('admin.depenses.update', $depense->id) }}" method="POST">
                    ...
                    <div style="margin-bottom: 20px;">
                        ...
                        ">
                            ...
                            @foreach($categories as $categorie)
                                <option value="{{ $categorie->id }}" {{ old('categorie_depense_id', $depense->categorie_depense_id) == $categorie->id ? 'selected' : '' }}>
                                    {{ $categorie->nom }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div style="margin-bottom: 20px;">
                        ...
                        <input type="text" name="libelle" id="libelle" value="{{ old('libelle', $depense->libelle) }}" required style="
                    ...
                    <div style="display: flex; gap: 20px; flex-wrap: wrap; margin-bottom: 20px;">
                        <div style="flex: 1; min-width: 200px;">
                            ...
                            <input type="number" name="montant" id="montant" value="{{ old('montant', $depense->montant) }}" required min="1" style="
                                width: 100%; padding: 12px 16px; border: 2px solid #e8e0d5; border-radius: 8px; font-size: 1rem; outline: none;
                            ">
                            @error('montant')
                                <div style="color: #dc3545; font-size: 0.85rem; margin-top: 5px;"><i class="fas fa-exclamation-circle"></i> {{ $message }}</div>
                        ...
                        <div style="flex: 1; min-width: 200px;">
                            ...
                            <input type="date" name="date_depense" id="date_depense" value="{{ old('date_depense', $depense->date_depense->format('Y-m-d')) }}" required style="
                                width: 100%; padding: 12px 16px; border: 2px solid #e8e0d5; border-radius: 8px; font-size: 1rem; outline: none;
                            ">
                            @error('date_depense')
                                <div style="color: #dc3545; font-size: 0.85rem; margin-top: 5px;"><i class="fas fa-exclamation-circle"></i> {{ $message }}</div>
                    ...
                    <div style="display: flex; gap: 20px; flex-wrap: wrap; margin-bottom: 25px;">
                        <div style="flex: 1; min-width: 200px;">
                            ...
                            ">
                                @foreach(['especes' => 'Espèces', 'cheque' => 'Chèque', 'virement' => 'Virement', 'mobile_money' => 'Mobile Money'] as $value => $label)
                                    <option value="{{ $value }}" {{ old('mode_paiement', $depense->mode_paiement) == $value ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                            @error('mode_paiement')
                                <div style="color: #dc3545; font-size: 0.85rem; margin-top: 5px;"><i class="fas fa-exclamation-circle"></i> {{ $message }}</div>
                        ...
                        <div style="flex: 1; min-width: 200px;">
                            ...
                            <input type="text" name="reference" id="reference" value="{{ old('reference', $depense->reference) }}" style="
                                width: 100%; padding: 12px 16px; border: 2px solid #e8e0d5; border-radius: 8px; font-size: 1rem; outline: none;
                            ">
                            @error('reference')
                                <div style="color: #dc3545; font-size: 0.85rem; margin-top: 5px;"><i class="fas fa-exclamation-circle"></i> {{ $message }}</div>
